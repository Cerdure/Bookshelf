$(function () {
  ...
  function bannerSlide(speed) {
    ...
    $('.banner-index-fill')
      .stop()
      .animate({ left: -57 + (count % 16) * 3.8 + 'px' }, 500)
    count++

    if (count == 17) {
      $('.banner-img-wrapper ul').animate({ right: -3405 + 160 * count + 'px' }, speed, function () {
        $('.banner-img-wrapper ul').css('right', '-3295px')
        count = 1
      })
    } else if (check && count == 1) {
      $('.banner-img-wrapper ul').animate({ right: '-685px' }, speed, function () {
        $('.banner-img-wrapper ul').css('right', '-3295px')
      })
    } else if (count < 3) {
    ...
    $('.active-origin-price').text(
      Number($('.bi-active').find('.book-origin-price').val()).toLocaleString('ko-KR') + '원'
    )
    $('.active-discount-rate').text('-' + $('.bi-active').find('.book-discount-rate').val() + '%')
    $('.active-discount-price').text(
      Number($('.bi-active').find('.book-discount-price').val()).toLocaleString('ko-KR') + '원'
    )
  ...
  function bannerSlideReverse(speed) {
    ...
    if (count == 0) {
      $('.banner-img-wrapper ul').animate({ right: -3455 + 160 * count + 'px' }, speed, function () {
    ...
    $('.banner-index-fill')
      .stop()
      .animate({ left: -57 + (count % 16 == 0 ? 15 : (count - 1) % 16) * 3.8 + 'px' }, 500)
  ...
  $(document).on('click', '.category-title span, .category-img-wrapper div', function () {
    switch ($(this).index()) {
      case 0:
      ...
      case 1:
        ...
        break
      case 2:
        $('html').stop().animate({ scrollTop: 1540 }, 1000)
        break
      case 3:
        $('html').stop().animate({ scrollTop: 2540 }, 1000)
        break
    }
  })
  ...
  $(window)
    .scroll(function () {
      ...
      if (st < 500) {
        ...
        $('.category-title span:nth-child(1)').stop().animate({ color: 'rgb(255, 190, 0)' }, 200)
        $('.category-title span:not(.category-title span:nth-child(1))').stop().animate({ color: '#696969' }, 200)
      } else if (st < 1500) {
        $('.category-title span:nth-child(2)').stop().animate({ color: 'rgb(255, 190, 0)' }, 200)
        $('.category-title span:not(.category-title span:nth-child(2))').stop().animate({ color: '#696969' }, 200)
      } else if (st < 2500) {
        $('.category-title span:nth-child(3)').stop().animate({ color: 'rgb(255, 190, 0)' }, 200)
        $('.category-title span:not(.category-title span:nth-child(3))').stop().animate({ color: '#696969' }, 200)
      } else {
        $('.category-title span:nth-child(4)').stop().animate({ color: 'rgb(255, 190, 0)' }, 200)
        $('.category-title span:not(.category-title span:nth-child(4))').stop().animate({ color: '#696969' }, 200)
      }
    })
    .resize(function () {
      st = $(this).scrollTop()
...
$(function () {
  ...
  $(window).scroll(async () => {
    if (checked) return
    if (dom('.check-wrapper').getBoundingClientRect().top < 100) {
  ...
  function checkFill(atdCount) {
    $('.check-fill')
      ...
      .css('animation', 'background-color-change linear 1s')
  }

  dom('.month-point-btn').addEventListener('click', async () => {
    anonymousBlock()
  ...
  dom('.month-coupon-btn')?.addEventListener('click', async () => {
