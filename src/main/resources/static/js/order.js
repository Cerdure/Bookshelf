$(() => {
  ...
  $(document).on("keyup", ".point-value", async e => {
    const allPoint = $(".rest").data("value");
    priceSum = $(".origin-sum").data("value") + $(".delivery-charge").data("value");
    usePoint = e.target.value > priceSum - couponVal ? priceSum - couponVal : e.target.value;
    restPoint = await restPointCheck(usePoint);
...
let priceSum, orderPrice, couponVal = 0, usePoint = 0, restPoint = -1;
...
function priceCalc() {
  ...
  $(".rest").attr("data-value", restPoint).text('잔여 포인트 : ' + restPoint.toLocaleString("ko-KR"));
  toggleClass(".all-point", "disable", restPoint == 0);
  toggle(".coupon-data", couponVal > 0);
  toggle(".point-data", usePoint > 0)
