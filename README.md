...
사이트도 지식도 대부분 백지 상태에서 시작하다보니, 직접 UI를 제작하고 애니메이션이나 기능을 구현해보면서 다양한 문제 상황에 직면했습니다. 그럴 때마다 이리저리 검색해보고 인과관계를 파악해 하나씩 대처해나가면서 문제를 해결하는 것에 익숙해지고, 나아가 처음에 목적했던 대로 JavaScript를 비롯한 프론트와 Spring Framework를 이용한 웹 개발을 이해하는 데 좋은 밑거름이 되었습니다. 여러가지로 부족하고 아쉬운 부분도 많지만 팀프로젝트 기간에 여러 동기들을 도움을 줄 수 있었던 것도 이 프로젝트 덕분이라는 생각이 듭니다.<br/><br/>
마지막까지 읽어주셔서 감사합니다.
